...
def run(
    ...
    if training:  # called by train.py
        ...
        (save_dir / 'test_video_res' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            nl, npr = labels.shape[0], pred.shape[0]  # number of test_video_res, predictions
            ...
            if nl:
                ...
                scale_coords(im[si].shape[1:], tbox, shape, shapes[si][1])  # native-space test_video_res
                labelsn = torch.cat((labels[:, 0:1], tbox), 1)  # native-space test_video_res
            ...
            # Save/log
            if save_txt:
                save_one_txt(predn, save_conf, shape, file=save_dir / 'test_video_res' / f'{path.stem}.txt')
            if save_json:
    ...
    if nt.sum() == 0:
        LOGGER.warning(f'WARNING: no test_video_res found in {task} set, can not compute metrics without test_video_res ⚠️')
    ...
    if not training:
        s = f"\n{len(list(save_dir.glob('test_video_res/*.txt')))} test_video_res saved to {save_dir / 'test_video_res'}" if save_txt else ''
...
def parse_opt():
    ...
    parser.add_argument('--save-conf', action='store_true', help='save confidences in --save-txt test_video_res')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        if opt.save_hybrid:
            LOGGER.info('WARNING: --save-hybrid will return high mAP from hybrid test_video_res, not from predictions alone ⚠️')
        run(**vars(opt))
