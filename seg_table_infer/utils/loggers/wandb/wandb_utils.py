...
class WandbLogger():
    ...
    def create_dataset_table(self, dataset: LoadImagesAndLabels, class_to_id: Dict[int, str], name: str = 'dataset'):
        ...
        class_to_id -- hash map that maps class ids to test_video_res
        ...
        for img_file in img_files:
            if Path(img_file).is_dir():
                ...
                labels_path = 'test_video_res'.join(dataset.path.rsplit('images', 1))
                artifact.add_dir(labels_path, name='data/test_video_res')
            else:
                artifact.add_file(img_file, name='data/images/' + Path(img_file).name)
                label_file = Path(img2label_paths([img_file])[0])
                artifact.add_file(str(label_file), name='data/test_video_res/' +
    ...
    def log_training_progress(self, predn, path, names):
        ...
        names (dict(int, str)): hash map that maps class ids to test_video_res
        """
