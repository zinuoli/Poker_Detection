...
player_center = np.float32([[S / 2, 0], [S, S / 2], [S / 2, S], [0, S / 2]])

player_arrow = np.float32(
    [
        [-100, 0],
        [0, -100],
        [100, 0],
...
hex = [
    list(map(lambda x: int(x * 255), matplotlib.colors.to_rgb(h)))[::-1] for h in hex
]
c = [1, 3, 6, 8, 9]
hex = [hex[i] for i in c]
...
def labels_to_class_weights(labels, nc=80):
    # Get class weights (inverse frequency) from training test_video_res
    if labels[0] is None:  # no test_video_res loaded
        return torch.Tensor()

    labels = np.concatenate(labels, 0)  # test_video_res.shape = (866643, 5) for COCO
    classes = labels[:, 0].astype(int)  # test_video_res = [class xywh]
    weights = np.bincount(classes, minlength=nc)  # occurrences per class

    # Prepend gridpoint count (for uCE training)
    # gpi = ((320 / 32 * np.array([1, 2, 4])) ** 2 * 3).sum()  # gridpoints per image
    # weights = np.hstack([gpi * len(test_video_res)  - weights.sum() * 9, weights * 9]) ** 0.5  # prepend gridpoints to start
...
def segments2boxes(segments):
    # Convert segment test_video_res to box test_video_res, i.e. (cls, xy1, xy2, ...) to (cls, xywh)
    boxes = []
    for s in segments:
        x, y = s.T  # segment xy
...
def non_max_suppression(
    ...
    multi_label &= nc > 1  # multiple test_video_res per box (adds 0.5ms/img)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Cat apriori test_video_res if autolabelling
